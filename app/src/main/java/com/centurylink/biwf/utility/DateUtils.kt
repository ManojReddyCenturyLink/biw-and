...
class DateUtils {
    companion object {
        ...
        fun formatAppointmentDate(dateInput: String): String {
            ...
            format =
                if (date.endsWith("1") && !date.endsWith("11")) SimpleDateFormat("MMM. d'st' yyyy", Locale.US)
                else if (date.endsWith("2") && !date.endsWith("12")) SimpleDateFormat("MMM. d'nd' yyyy", Locale.US)
                else if (date.endsWith("3") && !date.endsWith("13")) SimpleDateFormat("MMM. d'rd' yyyy", Locale.US)
        ...
        fun formatDateAssiaRequestFormat(dateInput: Long): String {
            var formattedDate: String = ""
            val output = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssZ", Locale.US)
            try {
                formattedDate = output.format(dateInput)
            } catch (e: ParseException) {
                e.printStackTrace()
            }
            return formattedDate
